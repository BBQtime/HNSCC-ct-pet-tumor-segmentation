...
def main(args):
    ...
    path_to_PFS_csv = pathlib.Path(config['path_to_PFS_csv'])
    ...
    # datasets:
    print('train_paths[:2]', train_paths[:2])
    ...
    if baseline:
        model = models.Encoder(in_channels, n_cls, n_filters)
    else:
        model = models.FastSmoothSENormDeepEncoder_supervision_skip_no_drop(in_channels, n_cls, n_filters, reduction)
