...
def main(args):
    ...
    path_to_save_dir = config['path_to_save_huber_dir']
    ...
    # test data paths:
    all_paths = sorted(utils.get_paths_to_patient_files(path_to_imgs=path_to_data, append_mask=False))
    ...
    data_set = dataset.HecktorDataset(val_paths, PFS_df, transforms=input_transforms, mode='test')
    ...
    # init predictor:
    path_to_save_pred = path_to_save_dir+'fold-'+fold+'/pred.csv'
...
if __name__ == "__main__":
    ...
    parser.add_argument("-b", "--best", type=str, required=False, default='best', help="if load best weight")
